#!/usr/bin/env bash
# ════════════════════════════════════════════════════════════════════════════
# run_scene_context_extraction.sh — run the full scene context extraction pipeline in one command.
#
...
#
# Output root: output/scene_context/<split>{,_processed}/<log_id>/<ts>.json
#
# Usage:
#   bash tools/scripts/run_scene_context_extraction.sh
#
# Optional env overrides:
#   SPLIT=val WORKERS=8 \
#   LOGS="3de5b5d6-...  02678d04-..."   # empty = whole split
#   CONFIG=tools/scene_context_extraction/config/settings.yaml   # alt settings.yaml
#   SKIP_VALIDATE=1                     # skip the schema-validation stage
#   PY=python
#
# Prerequisite: run inside the vLLM container / on a GPU host (the annotate
#   stage imports vllm). See tools/scene_context_extraction/docker/run_container.sh.
# ════════════════════════════════════════════════════════════════════════════
set -eo pipefail
...
if [[ -n "${CONFIG}" ]]; then
  ...
  _cfg_root="$(${PY} -c 'import sys; from tools.scene_context_extraction.src.loader import load_config; print(load_config(sys.argv[1])["paths"]["output_dir"])' "${CONFIG}" 2>/dev/null || true)"
  ROOT="${_cfg_root:-${ROOT}}"
fi
...
START=$(date +%s)
echo "[run_scene_context_extraction]  split=${SPLIT}  workers=${WORKERS}  logs=${LOGS:-ALL}"
...
# ── (1) vLLM annotation ────────────────────────────────────────────
banner "(1) annotate — tools.scene_context_extraction.annotate"
${PY} -m tools.scene_context_extraction.annotate \
  --split "${SPLIT}" "${CONFIG_ARG[@]}" "${LOGS_ARG[@]}"

# ── (2) post-processing (smoothing + confirmed-run dilation) ───────
banner "(2) postprocess — tools.scene_context_extraction.postprocess"
${PY} -m tools.scene_context_extraction.postprocess \
...
if [[ "${SKIP_VALIDATE}" == "1" ]]; then
  ...
  banner "(3) validate — tools.scene_context_extraction.validate"
  ...
  for d in "${VAL_DIRS[@]}"; do
    ...
    ${PY} -m tools.scene_context_extraction.validate "${d%/}"
  done
fi
