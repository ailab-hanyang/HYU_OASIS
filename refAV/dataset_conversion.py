...
def filter_ids_by_score(track_data):
    ...
    for category, category_ids in id_stats_by_category.items():
        ...
        if category in [
            # [FIX] Typo "REGULAR_VEHCILE" -> "REGULAR_VEHICLE" (E/I order). Mismatch caused topk=100 instead of 200.
            "REGULAR_VEHICLE",
            "PEDESTRIAN",
            "BOLLARD",
            "CONSTRUCTION_CONE",
...
def process_sequences(log_id, track_data, dataset_dir, base_output_dir, filter=True,
                      ego_offset=EGO_REAR_AXLE_OFFSET):
    ...
    if rows:
        ...
        add_ego_to_annotation(output_path.parent, output_path.parent, ego_offset=ego_offset)
        # output_path.unlink()
...
def pickle_to_feather(dataset_dir, input_pickle_path, base_output_dir="output",
    ...
    Args:
        ...
        base_output_dir: Base directory for output folders (e.g.
            output/tracker_predictions/<tracker>/<split>/). If <tracker> ends with
            "_ego", body-center ego offset is used automatically; otherwise the
            legacy rear-axle (0,0,0) is kept for back-compat.
        ego_offset: Override auto-detection. tuple (tx, ty, tz) in ego frame.
    """

    # print(f"Reading pickle file: {input_pickle_path}")

    if ego_offset is None:
        # base_output_dir = .../tracker_predictions/<tracker>/<split>
        # → parent.name == <tracker>
        tracker_name = Path(base_output_dir).parent.name
        ego_offset = (
            EGO_BODY_CENTER_OFFSET if tracker_name.endswith("_ego")
            else EGO_REAR_AXLE_OFFSET
        )
    ...
    with mp.Pool(max(1, int(0.9 * (os.cpu_count())))) as pool:
        pool.starmap(
            process_sequences,
            [
                (log_id, track_data, dataset_dir, base_output_dir, True, ego_offset)
                for log_id, track_data in data.items()
            ],
        )


def add_ego_to_annotation(
    log_dir: Path,
    output_dir: Path = Path("output"),
    ego_offset: tuple[float, float, float] = None,
):
    if ego_offset is None:
        ego_offset = EGO_REAR_AXLE_OFFSET
    ...
    ego_df["qy"] = 0
    ego_df["qz"] = 0
    ego_df["tx_m"] = ego_offset[0]
    ego_df["ty_m"] = ego_offset[1]
    ego_df["tz_m"] = ego_offset[2]
...
def filter_tracker_feathers_by_score(
    src_tracker_dir: Path,
    dst_tracker_dir: Path,
    split: str,
    ...
    written = 0
    for log_dir in sorted(src_split.iterdir()):
        if not log_dir.is_dir():
            continue
        src_fea = log_dir / "sm_annotations.feather"
        if not src_fea.exists():
            continue
        dst_fea = dst_tracker_dir / split / log_dir.name / "sm_annotations.feather"
        if dst_fea.exists():
            continue
        df = read_feather(src_fea)
        if "score" in df.columns:
            df = df[df["score"] >= score_threshold]
        dst_fea.parent.mkdir(parents=True, exist_ok=True)
        feather.write_feather(df.reset_index(drop=True), dst_fea)
        written += 1
    return written
...
def convert_log_prompt_df(
    ...
    annotations = read_feather(log_dir / "sm_annotations.feather")  # sm
    log_timestamps = np.sort(annotations["timestamp_ns"].unique())  # Timestamp
    all_uuids = list(annotations["track_uuid"].unique())            # Track UUID
